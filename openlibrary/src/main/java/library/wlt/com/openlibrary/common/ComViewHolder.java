package library.wlt.com.openlibrary.common;
...
public class ComViewHolder {
     ...
     * @return
     */
    public static ComViewHolder get(Context context, View convertView, ViewGroup parent, int layoutId, int position) {
        //如果为空  直接新建一个ViewHolder
        if (convertView == null) {
            return new ComViewHolder(context, parent, layoutId, position);
        } else {
            //否则返回一个已经存在的ViewHolder
            ComViewHolder viewHolder = (ComViewHolder) convertView.getTag();
            //记得更新条目位置
     ...
     * @return
     */
    public ComViewHolder setButton(int viewId, CharSequence text){
     ...
     * @return
     */
    public ComViewHolder setCheckBox(int viewId, boolean state) {
     ...
     * @return
     */
    public ComViewHolder setCheckBox(int viewId, CharSequence text, boolean state) {
        CheckBox box = getView(viewId);
        box.setText(text);
        box.setChecked(state);
        return this;
